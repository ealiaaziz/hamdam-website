...
- Platform: iPhone. Apple Watch companion app available. Available now on the AU App Store, App Store ID 6784461990. (Source: hamdam.com.au meta tags and App Store links)
  ...
  - What follows from that is narrower than it looks. **iPad is VERIFIED as a supported device. It is NOT yet approved marketing wording**, and the site still says iPhone: `availableOnDevice: 'iPhone'` in `src/lib/schema.js`, "Available now on iPhone" in the homepage description, and «برای آیفون» on the badge. The one place an iPad now appears is the Farsi Garden section, which uses Apple's own iPad panel because it is the only Farsi capture of that screen in existence -- Ealia's call on 2026-08-28, taken over shipping the section with no picture. Changing the site's platform wording is a separate decision he has not taken, so do not take it here.
