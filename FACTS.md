...
- **There is still no Farsi capture of the Garden screen**, in the listing or in `hamdam-ios/marketing` (newest set 2026-07-26, still says Discover). The Farsi Garden section therefore renders without a phone. One Farsi capture restores the two-column layout with no other change: pass it to `GardenSection.astro` and the `shot` branch does the rest.
