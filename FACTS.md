...
- Platform: iPhone. Apple Watch companion app available. Available now on the AU App Store, App Store ID 6784461990. (Source: hamdam.com.au meta tags and App Store links)
  ...
  - Approved phrasing: **"iPhone and iPad"**, and **"iPhone, iPad and Apple Watch"** where the watch belongs. Farsi: «آیفون و آیپد» and «آیفون، آیپد و اپل واچ». `آیپد` follows the app's own `آیفون`, written solid with no ZWNJ, rather than `آی‌پد`; `Localization.swift` is inconsistent on this (`آیفون` solid, `آی‌کلود` with) and the device-name precedent is the one that governs. Ealia should confirm the spelling once, because nothing in the app or the listing supplies it and this is the only Persian in the repo that was not copied from something he wrote.
