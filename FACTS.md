...
- Minimum iOS: 26. Lowered from 26.5 on 2026-08-15, when Ealia confirmed Hamdam 1.2 is live on the App Store. 1.2 carries `IPHONEOS_DEPLOYMENT_TARGET = 26.0`, which the 2026-08-13 entry below had already read out of the project file and queued against this exact event. Published as `operatingSystem: 'iOS 26+'` in src/lib/schema.js, which is pinned to that exact string by a test. Any change to the floor is made here first and in the schema second.
  ...
  - OPEN as of 2026-08-16: the live listing has not caught up, and the published `iOS 26+` is ahead of it. `itunes.apple.com/lookup` returns `version 1.1.1`, `minimumOsVersion 26.5`, `currentVersionReleaseDate 2026-08-06` on the AU, US and GB storefronts, and the AU product page still renders "Requires iOS 26.5 or later". Checked twice by two routes, so this is not a cache artefact of one endpoint. Ealia's 2026-08-15 confirmation is therefore about 1.2 being approved rather than released -- an approved build awaiting manual or phased release looks exactly like this from outside, and App Store Connect, which would say which, is not reachable from the site repo.
  - Decision 2026-08-16, Ealia's: leave `MINIMUM_IOS` at `26`. 1.2 lands imminently, at which point the published string becomes correct on its own, and churning the floor down and back up costs more than the gap. Recorded rather than fixed, deliberately. The gap is real while it lasts: a reader on iOS 26.0-26.4 sees `iOS 26+` on the site and cannot install from the listing. If 1.2 has still not appeared on a later reading of this line, that calculus has expired -- revert to `26.5` here first, `src/lib/appStore.js` second, the pinned test third, and re-lower when the listing itself says 26.0.
...
- **The homepage hero screenshot shows a screen the app cannot display. Raised 2026-08-16, needs Ealia, not fixed.** `src/assets/website-redesign/screenshots/EN/01-hero-en.png` is the hero image in `HeroCinematic.astro:24-33` and is reused by `ContextConstellation.astro:4-5`, captioned "Hamdam Today screen showing a morning verse by Hafez".
  ...
  - **Not swapped, because the honest replacement changes the hero's message.** `marketing/app-store-screenshots-2026-07-26/iphone-6.9-1320x2868/p69_01_today_en.png` in hamdam-ios is a genuine capture of the current Today screen, and it has no verse in it at all. Choosing between an accurate hero with no verse, a different real screen that does show a verse, and re-rendering the harness against the real view is Ealia's call, not a copy fix.
  ...
  - Consequence to check separately: the 2026-07-25 set this came from is the App Store screenshot set. If those were submitted, the same mismatch is on the listing.
