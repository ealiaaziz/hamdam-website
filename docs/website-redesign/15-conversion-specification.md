...
| 6 | ~~App Store URL and ID `6784461990`~~ | **Closed 2026-08-05.** Verified live against Apple, not just against the repo. The site's exact URL form, `https://apps.apple.com/au/app/id6784461990`, resolves 200 and redirects to `.../app/hamdam-daily-persian-poetry/id6784461990`, title "Hamdam: Daily Persian Poetry". `itunes.apple.com/lookup` on the AU storefront returns `trackId 6784461990`, `bundleId com.ealia.Hamdam`, v1.1, seller "Seyed Valiallah Azizollahi" (the same IP-owner name as item 8, expected). The bare-id form is canonical and Apple appends the slug itself, so the short URL in `src/lib/appStore.js` is correct and needs no slug. Campaign params survive the redirect: `?ct=web-hero` was still on the final URL, which is what makes the §11 attribution real rather than cosmetic. |
...
| 11 | "iOS 26+" requirement in structured data (`operatingSystem`, `src/pages/index.astro` and `src/pages/fa/index.astro`) | **Measured 2026-08-05, left unchanged by decision.** Apple's lookup API (`itunes.apple.com/lookup?id=6784461990`) reports `minimumOsVersion: 26.5` for the shipped v1.1 binary, so the published "iOS 26+" is currently permissive: a device on 26.0 to 26.4 reads the site as compatible but cannot install. Deliberately not corrected, because the app may lower its deployment target to 26.0, which would make the site's existing wording correct without an edit. Re-check when the next binary ships: if the target drops to 26.0 this row closes with no change; if it stays at 26.5, the structured data should say "iOS 26.5+". Note the "iOS 26+" in the privacy pages is a different claim (on-device reflection hardware, not the install minimum) and is unaffected either way. |
